...
export default function VehicleEditPage() {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      originCountryId: vehicle.originCountry?.id ?? "",
      fobPrice: vehicle.fobPrice,
      fobCurrency: vehicle.fobCurrency || "USD",
      vin: vehicle.vin ?? "",
      color: vehicle.color ?? "",
      mileage: vehicle.mileage ?? undefined,
      bodyType: vehicle.bodyType ?? "",
      fuelType: (vehicle.fuelType as VehicleFormValues["fuelType"]) ?? undefined,
      transmission:
        (vehicle.transmission as VehicleFormValues["transmission"]) ?? undefined,
      shippingCost: vehicle.shippingCost ?? undefined,
      insuranceCost: vehicle.insuranceCost ?? undefined,
      status: (vehicle.status as VehicleFormValues["status"]) ?? "TRANSIT",
      estimatedArrival: vehicle.estimatedArrival
        ? vehicle.estimatedArrival.slice(0, 10)
        : "",
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: VehicleFormValues) => {
      ...
      const { data } = await api.put<ApiResponse<Vehicle>>(
        `/vehicles/${vehicleId}`,
        payload
      )
      return data
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold tracking-tight">Aracı Düzenle</h1>
        {vehicle && (
